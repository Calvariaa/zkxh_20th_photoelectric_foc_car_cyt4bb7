#include "vofaplus.h"

float data_send[DATA_SEND_SIZE] = {0};
int16_t data_send_list[DATA_SEND_QUEUE_SIZE][3] = {0};

void send_vofaplus()
{
    for (int8_t i = 0; i <= DATA_SEND_SIZE; i++)
        printf("%f,", data_send[i]);
    printf("-1.0\r\n");
}

uint32_t data_send_number = 0;
uint32_t data_send_head = 0;
uint32_t data_send_tail = 0;
void send_vofaplus_queue()
{
    if (data_send_number > 0)
    {
        printf("%d,%d,%d,%d,%d,%d\r\n", data_send_number, data_send_head, data_send_tail, data_send_list[data_send_head][0], data_send_list[data_send_head][1], data_send_list[data_send_head][2]);
        data_send_head = (data_send_head + 1) % DATA_SEND_QUEUE_SIZE;
        data_send_number--;
    }
    else{
        printf("No Data\r\n");
    }
}

void data_send_add(int16_t _data1, int16_t _data2, int16_t _data3)
{
    data_send_list[data_send_tail][0] = _data1;
    data_send_list[data_send_tail][1] = _data2;
    data_send_list[data_send_tail][2] = _data3;
    data_send_tail = (data_send_tail + 1) % DATA_SEND_QUEUE_SIZE;
    data_send_number++;
}